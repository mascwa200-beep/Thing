...
/**
 * Fetching expansion packs from where they are published — the Android twin of the desktop
 * `PackRepository`, same shape for the same reasons.
 *
 * Packs ride the same machinery as the app updates: a rolling GitHub release whose assets are the
 * archives, plus a small `packs.json` catalog beside them. That is deliberate reuse — the repository
 * is private, so an asset needs the same authenticated request `UpdateRepository` already gets
 * right, and rediscovering the private-asset quirk in a second place is how one of them ends up
 * broken.
 *
 * This class is only the wire between pieces that already exist and are tested: [ContentPack]'s
 * rules, [PackArchive]'s reader and [PackStore]'s atomic install.
 *
 * ⚠️ **A catalog entry names an asset, not a URL.** The catalog is content and the URL is
 * infrastructure; letting a published content file dictate where the app makes authenticated
 * requests would be a way to point it anywhere. The asset name is resolved against the release's own
 * asset list, and an entry that resolves to nothing is simply not offered — a download button that
 * always fails costs more trust than a pack not being there yet.
 */
class PackRepository(
    ...
    private suspend fun headers(): Map<String, String> = buildMap {
        ...
        // The same lesson the updater learned the hard way: GitHub serves authenticated API responses
        // with max-age=60, so a caching client answers from them and a pack published a moment ago is
        // missed while the app reports there is nothing new.
    ...
    suspend fun install(
        ...
        runCatching {
            val dir = File(cacheDir, "packs").apply { mkdirs() }
            val dest = File(dir, "${ContentPack.qualify(pack.id, "archive")}.bin")
            try {
                http.download(
                    url = pack.url,
                    dest = dest,
    ...
    companion object {
         ...
         *
         * Identical to the desktop's by design, so "a catalog names an asset, never a URL" is one
         * rule rather than two that can drift. A catalog is published by hand as often as not, and a
         * row naming an asset that was never uploaded is the ordinary mistake.
         */
