    ...
    PullToRefreshBox(
        ...
        when {
            ...
            state.isError -> ErrorState(state.error ?: "Error", onRetry = { vm.refresh() })
                ...
                orbitalSections(state.data, state.stale, c)
            }
        }
    }
}

/** Orbital readouts as LazyColumn sections so they can be mixed into the combined DATA feed. */
fun LazyListScope.orbitalSections(d: OrbitalData?, stale: Boolean, c: NightwirePalette) {
