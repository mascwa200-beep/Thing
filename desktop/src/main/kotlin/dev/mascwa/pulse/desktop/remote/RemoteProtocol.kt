 ...
 * ## Mirror note
 *
 * This is a copy-adapted mirror of `core/telemetry/.../RemoteProtocol.kt` — **only the package line differs**.
 * The repo's deliberate convention is to copy rather than share a multiplatform source set (see the module
 * header in `desktop/build.gradle.kts`), because flipping `:core:telemetry` to a real multiplatform target is
 * a structural risk to the working Android build. The two sides MUST stay byte-identical below the package
 * line: they are the two ends of one wire protocol, and the mirrored test suite runs on both to prove it.
 */
