...
OUT=/home/user/Thing/scratchpad/interr/out; rm -rf "$OUT"; mkdir -p "$OUT"
T=/home/user/Thing/core/telemetry/src/main/java/dev/mascwa/pulse/core/telemetry
java -cp "$KC:$STD:$TRV:$ANN:$COR" org.jetbrains.kotlin.cli.jvm.K2JVMCompiler \
  -nowarn -d "$OUT" -cp "$STD:$COR" \
  $T/GuideSearch.kt $T/LibraryConsult.kt \
  /home/user/Thing/scratchpad/interr/Probe.kt 2>&1 | grep -Ev '^(warning|info):' || true
java -cp "$OUT:$STD:$COR" ProbeKt
