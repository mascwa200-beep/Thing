#!/usr/bin/env bash
# runmain.sh plus robolectric's android-all, for a probe that needs the REAL SensorManager maths.
#   ./runandroid.sh PoleProbe.kt dev.mascwa.pulse.core.telemetry.PoleProbeKt
# Same jar discipline: every jar asserted, and a compiler that produced no classes is reported
# rather than surfacing later as "class not found".
set -euo pipefail
cd "$(dirname "$0")/../.."
...
AJAR=/tmp/android-compile-check/android-all-15-robolectric-13954326.jar
...
for j in "$KC" "$STD" "$TRV" "$ANN" "$AJAR" "$COR" "$JSOUP" "$SER" "$SERJ" "$SPLUG"; do
  [ -n "$j" ] && [ -f "$j" ] || { echo "missing a required jar: '$j'"; exit 2; }
done
[ $# -ge 2 ] || { echo "usage: runandroid.sh <Probe.kt> <MainClass>"; exit 2; }
PROBE="$1"; shift
MAIN="$1"; shift
...
OUT=$(mktemp -d)
# ⚠️ android-all LAST. It carries stub bodies for a great deal of the JDK too, and putting it ahead
# of kotlin-stdlib is how a probe ends up resolving java.lang against the stubs.
TARGET_CP="$STD:$JSOUP:$SER:$SERJ:$AJAR"
