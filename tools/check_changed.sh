...
# ⚠️ Every changed file, not just the Kotlin, and hoisted ABOVE the early exit below. Two of the
# gates here — the control-byte one and the workflow-expression one — are not about Kotlin at all,
# and the script used to return "no changed Kotlin files" and stop. So a commit that touched only a
# workflow ran NOTHING, which is precisely the commit that broke the build the day the workflow gate
# was written. Found by running this on that very change.
if [ "${1:-}" = "--staged" ]; then
...
if [ -z "$CHANGED" ] && [ -z "$ALLCH" ]; then
    echo "nothing changed"
    exit 0
fi

if [ -z "$CHANGED" ]; then
    echo "== no changed Kotlin files; running the gates that are not about Kotlin =="
else
...
# ---- 0b. workflow expressions ---------------------------------------------------------------------
# ⚠️ Second, and it is not a Kotlin gate at all, because the failure it catches gives you nothing to
# read. An invalid expression makes a run complete in under a second with ZERO jobs — no log, no
# failing step, just "failure" — and `yaml.safe_load` passes the file, because expressions are opaque
# strings to YAML and a separate language to GitHub. Two runs were spent on an EMPTY `${ { } }`
# written inside a `#` comment in a `run:` block, where it is a comment to the shell and an
# expression to GitHub.
echo
echo "== workflow expression gate =="
WF=$(echo "$ALLCH" | grep -E '^\.github/workflows/.*\.ya?ml$' || true)
if [ -z "$WF" ]; then
    echo "   (no workflow files changed)"
else
    # shellcheck disable=SC2086
    if OUT=$(python3 tools/workflow_expr_check.py $WF 2>&1); then
        echo "   ok    $OUT"
    else
        echo "$OUT" | sed 's/^/   /'
        FAIL=1
...
PKGS=$([ -n "$CHANGED" ] && echo "$CHANGED" | xargs -r -n1 dirname | sort -u | grep -v '^scratchpad/' || true)
for d in $PKGS; do
...
if [ -z "$PKGS" ]; then
    echo "   (no Kotlin packages touched)"
    DUP="no colliding"
else
...
# ⚠️ Handed no files, kotlinc reports its own usage rather than nothing, which would read as a
# finding. Skipped explicitly instead.
if [ -z "$CHANGED" ]; then
    PARSE=""
